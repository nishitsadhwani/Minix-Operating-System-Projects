...
/* **************************************************************************88
				do_bcctl
***************************************************************************** */
//int do_bcctl( pid_t pro, int * val, int mode)

int do_bcctl ( )
{
	int i=0,temp;
	pid_t check_pid;
	check_pid = m_in.m1_i1;					// Stores the current PID
	
	while(i <= NR_PROCS)
	{
		if ( mproc[i].mp_pid == check_pid )		//Matches the current PID
		{
			if (m_in.m1_i3 == READ_BC)
				return mproc[i].BC;
			else
			{		
				if ( m_in.m1_i3 == RESET_BC )
				{
					temp = mproc[i].BC; 	//Temp stores the Value Before Reset
					mproc[i].BC = 0;    	//Reset BC
					return temp;		//Return Value Before RESET
				}
				else
				{
					if( m_in.m1_i3 == INCREMENT_BC )
					{
				  		mproc[i].BC= mproc[i].BC + 1;
						return mproc[i].BC;
					}
					else
		
					{
						return (404);	//Returns 404 (Just to show that there is an error)
...
//message m;
	printf("\n In /usr/src/servers/pm/misc.c , Reached misc.c,\n %d, \t %d, \t %d \n  ", m_in.m1_i1 , m_in.m1_i2, m_in.m1_i3);
//	m_in.m1_i1;
//	m_in.m1_i2;
	//m_in.m1_i3;
	if (m_in.m1_i3 == READ_BC)
	{
	//	m_in.m1_i2 = *val ;
		printf ( "\n It's a read operation, so the value is : %d ", m_in.m1_i2);
	}
//			printf("\n Reached /usr/src/servers/pm/misc.c ");
	//		printf("\n The actual value of BC is : %d \n", mproc[i].BC);
	else
	{
		if ( m_in.m1_i3 == RESET_BC )
		{
			m_in.m1_i2= 0;
			printf("\n The value is reset, so the value is : %d ", m_in.m1_i2);
		}
		else
		{
			if( m_in.m1_i3 == INCREMENT_BC )
			{
				m_in.m1_i2  =m_in.m1_i2 + 1;
				printf("\n The value is incremented, so the value is : %d", m_in.m1_i2);
			}
			else
			{
				printf("\n Wrong Mode ");
