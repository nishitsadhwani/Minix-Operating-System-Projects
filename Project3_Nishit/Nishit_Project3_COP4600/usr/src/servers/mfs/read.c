...
int fs_readwrite(void)
{
  ...
  unsigned int off, cum_io, block_size,seq_block_size, chunk;//-------------
  mode_t mode_word;
  mode_t seq_file;		//------------------------------------------
  int completed;
  struct inode *rip;
  ...
  seq_file = ( mode_word == I_SEQ ? 1: 0 ); //----------------------------- 

  regular = (mode_word == I_REGULAR || mode_word == I_NAMED_PIPE || mode_word == I_SEQ);
  block_spec = (mode_word == I_BLOCK_SPECIAL ? 1 : 0);
  sequential = (mode_word == I_SEQ);        //-----------------------------
  ...
  seq_block_size = block_size - sizeof(zone_t); //---------------------- 
  ...
  if (rw_flag == WRITING && !block_spec) {
	...
	//  if(position > f_size) clear_zone(rip, f_size, 0);//-----
	  ...
	  if(position > f_size && !sequential) clear_zone(rip, f_size, 0);//----
  }
  ...
  while (nrbytes > 0) {
	...
	if(sequential)
	{
	  off = ((unsigned int) position) % seq_block_size; /* offset in blk*/
	  chunk = min(nrbytes, seq_block_size - off);
	}
	//------------------------------------------------------
	  if (rw_flag == READING) {
...
/* Read or write (part of) a block. */
  ...
  *completed = 0;
  ...
  sequential = (rip->i_mode & I_TYPE) == I_SEQ;//-----------------
  ...
  if (rw_flag == READING) {
	...
	if(sequential)
		r = sys_safecopyto(VFS_PROC_NR, gid, (vir_bytes) buf_off,
		   (vir_bytes) (b_data(bp)+sizeof(zone_t)+off), (size_t) chunk);
	else
		r = sys_safecopyto(VFS_PROC_NR, gid, (vir_bytes) buf_off,
  ...
  //------------------------------------------------------------------------- 
   if(sequential) 
    n = (off + chunk + sizeof(zone_t)== block_size ? FULL_DATA_BLOCK : PARTIAL_DATA_BLOCK);
    else
   n = (off + chunk == block_size ? FULL_DATA_BLOCK : PARTIAL_DATA_BLOCK);
   //------------------------------------------------------------------------
  put_block(bp, n);
...
# define BLOCKS_MINIMUM		(nr_bufs < 50 ? 18 : 32)
  ...
  if (block_spec) 
	...
	dev = rip->i_dev;
 //if sequential, don't do read ahead-------------------------
  if(sequential)
